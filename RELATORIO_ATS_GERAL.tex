...
A consolidacao foi feita com tres objetivos centrais: preservar a rastreabilidade técnica de todas as decisoes de teste, comparar de forma objetiva os resultados obtidos em duas toolchains diferentes e fornecer um artefacto unico, completo e reprodutivel para avaliacao.

A estrategia global combinou reforco manual de testes unitarios, geracao automatica de testes (EvoSuite e apoio por IA), analise estrutural por cobertura, analise de robustez da suite por mutação, property-based testing e automação de pipeline com repetibilidade de execucao.
...
Do ponto de vista de teste, o foco foi consolidar a suite JUnit existente, aumentar cobertura em utilitarios e fluxos criticos de controlador e integrar EvoSuite por perfis Maven.
...
Do ponto de vista de teste, o foco foi reforco de testes em utilitarios e regras de negocio, medicao sistematica de cobertura, uso de mutação para localizar fragilidades da suite e automação integral em tarefas Gradle e script unico.
...
\begin{itemize}
    ...
    \item registar limitações externas sem perder validade metodologica;
    \item integrar assistência por IA com revisao manual obrigatória.
\end{itemize}

\section{Conformidade explicita com o enunciado ATS 2025/2026}
O enunciado ATS define 6 tarefas obrigatórias e 4 extras valorizados. Nesta secção, o cumprimento e apresentado de forma rastreavel, com evidencia técnica por projeto.
...
\midrule
1 & Escrever/complementar testes JUnit & Reforco em utilitarios e controlador & Reforco em utilitarios e estatisticas & Cumprido \\
2 & Usar EvoSuite para geracao automatica & Integrado via perfil Maven (escopo reduzido) & Integrado via task Gradle (escopo reduzido) & Cumprido \\
3 & Analisar cobertura e qualidade dos testes & Analise qualitativa e diagnostico por perfis & JaCoCo + interpretacao de qualidade via PIT & Cumprido \\
4 & Usar PIT para criar mutantes & Nao foco principal nesta iteracao & PIT integrado com métricas reportadas & Cumprido (global) \\
5 & Usar QuickCheck/Hypothesis (ou equivalente) & Nao foco principal nesta iteracao & jqwik aplicado (equivalente PBT em Java) & Cumprido (global) \\
6 & Relatorio com graficos e analise & Consolidado neste relatorio geral & Consolidado neste relatorio geral & Cumprido \\
\bottomrule
\end{tabular}
...
\midrule
1 & automação com Maven/Makefile/comando unico & Pipeline Gradle agregada (\texttt{atsFullPipeline}) e comandos reproduziveis por etapa & Cumprido \\
2 & Refactoring + teste de regressao & Refatoracao behavior-preserving em modulo de estatisticas (Projeto 2) com revalidacao da suite & Cumprido \\
3 & Property-Based Testing em Java & jqwik aplicado para propriedades/invariantes de comportamento & Cumprido \\
4 & Bom relatorio & Documento geral consolidado, com comparacao, evidencias, riscos e anexos tecnicos & Cumprido \\
\bottomrule
\end{tabular}
...
O PDF anexado inclui tambem o enunciado original de POO (24/25), com requisitos funcionais de negocio. A estrategia de teste procurou cobrir especialmente as zonas de maior risco funcional:
\begin{itemize}
    \item persistencia e serializacao de estado;
    \item estatisticas sobre reproducoes/utilizadores/playlists;
    \item regras de planos e comportamento de entidades de dominio;
    \item fluxos de criacao, autenticacao e reproducao em menus/servicos.
\end{itemize}

Esta abordagem nao substitui validacao exaustiva de todos os caminhos da aplicacao, mas alinha a cobertura com os criterios de avaliacao de ATS e com os pontos de maior impacto no comportamento observado.

\section{Porque manter Maven num projeto e Gradle no outro}
A coexistencia de Maven e Gradle foi uma decisao deliberada para explorar ecossistemas complementares. No Projeto 1, \textbf{Maven} foi mantido por favorecer configuracao declarativa e separacao por \textit{profiles}, util para cenarios com modos de execucao distintos (suite normal, runtime EvoSuite e geracao EvoSuite). No Projeto 2, \textbf{Gradle} foi mantido por facilitar composicao de tarefas ATS e automação de pipeline end-to-end, incluindo relatorios em cadeia.
...
\subsection{JUnit}
JUnit foi a base comum dos dois projetos, com enfase em cenarios de fronteira, validacao de excecoes e caminhos negativos, e garantia de regressao funcional apos alteracoes.
...
Para reforcar a componente academica do relatorio, a tabela seguinte liga areas funcionais do enunciado POO aos focos de teste considerados.

\begin{table}[H]
\centering
\begin{tabular}{p{0.38\textwidth}p{0.26\textwidth}p{0.28\textwidth}}
\toprule
Area funcional do enunciado POO & Evidencia no Projeto 1 & Evidencia no Projeto 2 \\
\midrule
Gestao de entidades e persistencia de estado & Testes de serializacao e import/export via controlador & Testes de parsing/adapters e serializacao de dados \\
Registo e validacao de interacoes de utilizador & Testes de login/registo e cenarios de erro & Testes de comportamento em utilitarios e regras de apoio \\
Estatisticas sobre estado do sistema & Cobertura parcial por testes de controlador/modelo & Foco explicito em testes de \texttt{Stats} e propriedades \\
Regras especificas de dominio (playlists, tipos de musica, planos) & Cobertura seletiva em classes de modelo alvo de EvoSuite/JUnit & Cobertura em adapters, playlist invariants e regras agregadas \\
Robustez de erros/excecoes & Testes negativos (URL, IO, input invalido) & Testes de fronteira e robustez orientados por risco \\
\bottomrule
\end{tabular}
...
\subsection{Justificacao técnica desta limitação}
A geracao automatica em todas as classes de dominio tem custo elevado e pode alongar excessivamente cada iteracao. A limitação para 3 classes permite:
\begin{itemize}
...
A tarefa de EvoSuite usa container Docker e pode encontrar limitações de compatibilidade de bytecode dependendo da imagem/runtime em uso, especialmente em projetos com Java mais recente. Esta e uma limitação externa de toolchain e nao invalida a integração técnica nem o racional de uso.
...
\begin{figure}[!h]
    ...
    \caption{Resumo da execucao JUnit no Projeto 2}
...
Os tres graficos acima foram gerados localmente a partir das métricas consolidadas desta iteracao para substituir capturas de ecra que nao estavam disponiveis no workspace. A informacao reportada foi preservada (JUnit, JaCoCo e PIT), com foco em legibilidade para avaliacao academica.

\section{Geracao automatica de testes com assistência por IA}
...
\midrule
Sistema de build & Ciclo declarativo + perfis & Tarefas composiveis + automação agregada \\
...
mutação & Nao foco principal nesta iteracao & PIT integrado e operacional \\
...
\section{Riscos, limitações e Mitigacoes}
...
\begin{itemize}
    ...
    \item limitação de escopo EvoSuite para iteracoes mais curtas;
    \item reforco incremental orientado por métricas (JaCoCo/PIT);
...
\section{Conclusao}
O relatorio geral evidencia uma evolucao técnica consistente nos dois projetos, com abordagens complementares: no Projeto 1, consolidacao da suite e integração por perfis Maven, e no Projeto 2, pipeline ATS completa com cobertura, mutação e automação.

A decisao de aplicar EvoSuite a apenas 3 classes por projeto foi deliberada e adequada ao prazo, permitindo equilibrio entre profundidade técnica e tempo de execucao.

A assistência por IA contribuiu para acelerar geracao e refinamento de testes, mantendo rigor atraves de revisao manual e validacao por execucao local. Com isso, o processo resultou em maior produtividade sem comprometer qualidade metodologica.

Em sintese, os dois projetos encontram-se com base solida para avaliacao ATS, com evidencias reproduciveis, comparacao técnica clara e roteiro concreto de melhoria contínua.
...
Justificacao técnica e analise critica & Sim & Sim \\
\bottomrule
\end{tabular}
...
Falsos positivos em sugestoes de IA & Media & Medio & Revisao manual obrigatória e execucao local antes de aceitar alteracoes \\
\bottomrule
\end{tabular}
